                    ...
                    @foreach($members as $member)
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b">
                                ...
                                <form action="{{ route('member.borrowBook', $member->id) }}" method="POST" class="inline-block mt-2">
                                    @csrf
                                    <select name="book_id" class="w-full p-2 border border-gray-300 rounded" required>
                                ...
                                @if($member->books->isNotEmpty())
                                    <form action="{{ route('member.returnBook', $member->books->first()->id) }}" method="POST" class="inline-block mt-2">
                                        @csrf
                                        <button type="submit" class="bg-gray-500 text-white px-4 py-2 rounded">Return Book</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
